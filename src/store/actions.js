...
export const fetchPosts = async ({ commit, state }) => {
  if (await dateArchivePosts({ commit, state })) {} else if (await homePosts({ commit, state })) {} else if (await taxonomyArchivePosts({ commit, state })) {} else if (await AuthorArchivePosts({ commit, state })) {} else if (await singularPost({ commit, state })) {}
}
